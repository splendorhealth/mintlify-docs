...
<Steps>
  <Step title="Create a hosted source">
    ...
    <Note>
      To enable **semantic (vector) search** on this data, also pass `semantic_config.document_embeddings.fields` here, naming the fields to embed. Without it, only keyword and SQL search work — semantic queries fall back to keyword results. See [Run a semantic search](/guides/semantic-search#configure-embeddings).
    </Note>
  </Step>
