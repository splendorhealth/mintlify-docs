...
Authenticate with the source's **ingest token** as a bearer credential — the token already names the source and tenant, so no `X-Splendor-Tenant-Id` header is needed.
