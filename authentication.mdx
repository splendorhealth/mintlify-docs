...
You get both values from the [console](https://app.splendor.io). Treat the token as a secret: never embed it in client-side code or commit it to version control.
...
<CodeGroup>
```bash curl
curl https://api.splendor.io/v1/me \
  -H "Authorization: Bearer $SPLENDOR_TOKEN"
```
...
See [Roles & permissions](/concepts/overview) for the full breakdown of which operations require which role.

{/* TODO: link to the dedicated Roles, permissions & plans pages once they land. */}
...
{/* TODO: confirm the production API base URL (no custom domain is configured in infra yet). */}
